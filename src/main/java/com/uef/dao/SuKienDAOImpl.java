    ...
    public List<SuKien> getAll() throws Exception {
        ...
        try (Connection conn = DBConnection.getConnection(); PreparedStatement ps = conn.prepareStatement(sql); ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                ...
                Timestamp ngayGio = rs.getTimestamp("ngay_gio");
                e.setNgayGio(ngayGio != null ? ngayGio.toLocalDateTime() : null);

                Timestamp hanDangKy = rs.getTimestamp("han_dang_ky");
                e.setHanDangKy(hanDangKy != null ? hanDangKy.toLocalDateTime() : null);
